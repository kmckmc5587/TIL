...
```sql
SELECT COUNT(BillingPostalCode) '송장수' FROM invoices WHERE BillingPostalCode NOT NULL;
```
...
```sql
.shema albums
SELECT ArtistId, COUNT(*) '앨범 수' FROM albums GROUP BYArtistId ORDER BY COUNT(*) DESC LIMIT 1;
```
...
```sql
SELECT LastName, FirstName, (strftime('%Y', 'now') - BirthDate + 1) '나이' FROM employees ORDER BY EmployeeId;
```
